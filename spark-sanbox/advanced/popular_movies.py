...
with SparkSession.builder.appName("PopularMovies").getOrCreate() as spark:
    ...
    user_ratings = (
        spark.read.option("sep", "\t").schema(schema).csv("././datasets/ml-100k/u.data")
    )

    movie_ratings = user_ratings.groupBy("movie_id").count()

    lookup_name_udf = func.udf(lookup_name)

    movie_with_name = movie_ratings.withColumn(
        "title", lookup_name_udf(func.col("movie_id"))
