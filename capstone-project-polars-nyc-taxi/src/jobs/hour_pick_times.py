...
def extract_time(dataset: pl.LazyFrame, time_types: List[str]) -> pl.LazyFrame:
    ...
    for time_type in time_types:
        if time_type not in time_dict:
            raise ValueError(f"Invalid time_type argument. Expected one of: {list(time_dict.keys())}")

    return dataset.with_columns(
        *[time_dict[time_type] for time_type in time_types]
    )


def rank_by_column(dataset: pl.LazyFrame, column_name: str, over: str, n: int) -> pl.LazyFrame:
    """
...
if __name__ == '__main__':
    ...
    try:
        with pl.Config(tbl_rows=100):
            taxi_dataset = (
                pl.scan_parquet(str(DATASET_FOLDER / "yellow_*.parquet"))\
            ...
            filtered_dataset = (
                time_taxi_dataset
                .filter(pl.col("tpep_pickup_datetime").dt.year() >= YEAR_FILTER)
            )

            record_count_dataset = (
                filtered_dataset.groupby(["month", "hour"]).count()
            )
