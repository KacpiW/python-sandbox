...
import logging
import polars as pl
from polars import LazyFrame, read_csv, scan_csv, scan_parquet, Config, col, Int64, DataFrame

from src.jobs.config import ROOT_FOLDER, LOGGING_FORMAT, YEAR_FILTER, DATASET_FOLDER

LOCATION_NAMES = scan_csv(str(ROOT_FOLDER / "dataset" / "dictionaries" / "taxi_zone_lookup.csv"))
TOP_N_RECORDS = 5
...
def create_named_locations_dataframe(dataset: LazyFrame, location_names: LazyFrame = LOCATION_NAMES) -> LazyFrame:
    """
...
def extract_top_pickup_locations_per_month(dataset: LazyFrame, n: int) -> LazyFrame:
    ...
    """
    selected_columns = (
        dataset.select("tpep_pickup_datetime", "PULocationID")
    )
    with_extra_columns = (
        selected_columns
        .with_columns(
    ...
    filtered = (
        with_extra_columns
        .filter(col("tpep_pickup_datetime").dt.year() >= YEAR_FILTER)
        .with_columns(col("PULocationID").cast(Int64))
    )
    named_locations = (
        create_named_locations_dataframe(filtered)
    )
    aggregated_values = (
        named_locations.groupby(["tpep_pickup_month", "Borough"]).count()
    )
...
def main():
    ...
    try:
        with Config(tbl_rows=100):
            ...
            query = (
                extract_top_pickup_locations_per_month(dataset, TOP_N_RECORDS)
            )
