...
with SparkSession.builder.appName("AverageDistance").getOrCreate() as spark:
    ...
    try:
        # Load data
        taxi_data = spark.read.format("parquet").option("header", "true")\
        ...
        if num_records > 0:
            ...
            average_distance_per_month = taxi_data\
            ...
            for distance in average_distance_per_month.collect():
                print(f"Month: {distance.month}, Average Distance: {distance.average_distance_km} km")
