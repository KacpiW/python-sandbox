...
class OtoMotoScraper(scrapy.Spider):
    ...
    cars offers, as well as method to retrive all possible data iprovided by sellers,
    thus parsed results may differ in number of acttributes in case some seller
    provided all
    ...
    def parse(self, response):
        ...
        logger.info("Got successful response from {}".format(response.url))
        ...
        offer_links = [
            link.get("href")
            for link in offers.find_all("a", attrs={"class": "offer-title__link"})
        ]

        # Iterating over offers found on a page
        for item_url in offer_links:
            if "carsmile" in item_url:
                logger.warning("A website with carsmile string has been skipped.")
                continue
            else:
        ...
        try:
            next_page = (
                soup.find("li", attrs={"class": "next abs"}).find("a").get("href")
            )
    ...
    def parse_offer_details(self, response):
        ...
        """
        logger.info("Got successfull response from {html}".format(html=response.url))
        ...
        try:
            ...
            details_dict["lokalizacja"] = soup.find(
                "span", attrs={"class": "seller-box__seller-address__label"}
            ).text.strip()
            ...
            details_dict["wyposazenie"] = [feature.text.strip() for feature in features]
    ...
    def save_to_file(self, to_save):
        ...
        file_path = (
            save_path + str(datetime.date.today().strftime("%d_%m_%Y")) + ".json"
        )
        ...
        if os.path.isfile(file_path):
            with open(file_path, mode="a") as out_file:
                ...
                out_file.write("\n")
                logger.info("Appended new data to already existing json file.")

        else:
            with open(file_path, mode="w") as out_file:
                json.dump(to_save, out_file)
                out_file.write("\n")
                logger.info("Created and added first line of a data to json file.")
